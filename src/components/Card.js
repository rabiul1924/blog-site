...
const Card = ({ post }) => {
    return (
        <Link to={`/blog/${post._id}`} className="p-2 cursor-pointer block text-center md:text-left">
            <img src={post.photo} alt={post.name} className="h-52 w-full" />
            <p className="uppercase text-sm tracking-wider my-2 text-gray-400">{post?.date}</p>
